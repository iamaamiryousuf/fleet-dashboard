  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-lg text-xs">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="font-medium text-slate-700">{formatCurrency(payload[0]?.value)}</span>
      </div>
    </div>
  );
};
...
export function MonthlyTrendChart({ data }) {
  const chartData = useMemo(() => {
    const grouped = aggregateByField(data, "month", ["totalCost", "monthlyBilling"]);
  ...
  return (
    <div className="card p-5">
      <div className="mb-4">
        <h3 className="font-display font-bold text-slate-800 text-sm">Monthly Expense Trend</h3>
        <p className="text-xs text-slate-400 mt-0.5">Total operational cost</p>
      </div>
